...
- Seedream 5.0 Pro art-directs a niche-matched, text-free background for each of the three main-cover variants. Browser Canvas then places only the selected completed sticker PNGs, exact quantity and sales copy, so the model cannot invent, redraw, duplicate or crop the product designs.
...
- OpenAI selects 15 representative real designs for three high-impact crop-safe cover variants; selection is read-only and can never replace a sticker or trigger a Seedream request. Canvas preserves the exact sticker pixels and prevents invented product art. The listing set also includes a deterministic “What You Receive” infographic and enlarged transparency/edge-quality proof.
