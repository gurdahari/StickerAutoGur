...
- Seedream 5.0 Pro receives 12 selected real sticker PNG references plus a deliberately short customer-facing title, quantity and selling points, then generates each complete main cover—typography, badges, background and sticker composition—in one cohesive render. Long market-research and 100-design briefs are never exposed as cover text. The former exact-pixel Canvas cover remains available only as a provider-failure fallback.
